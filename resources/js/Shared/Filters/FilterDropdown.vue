<template>
  <div class="flex items-center space-x-3 w-full md:w-auto">
    <button id="filterDropdownButton" data-dropdown-toggle="filterDropdown"
      ...
      type="button">
      <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="h-4 w-4 mr-2 text-gray-400" viewbox="0 0 20 20"
        fill="currentColor">
    ...
    <div id="filterDropdown" class="z-10 hidden p-3 bg-white rounded-lg shadow dark:bg-gray-700 w-fit">
      <div v-for="filter in filterByFiltersEnabled" :key="filter">
        ...
        <ul class="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
          <div class="space-y-2 text-sm" v-for="(option, index) in filter.options" :key="index">
            <li class="flex items-center">
              <input type="checkbox" :id="option[filter.valueKey]" :value="option[filter.valueKey]"
                @change="filterByChange()" v-model="filtersDefault[filter.slug]"
                class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
              <label for="unpaid" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">{{
